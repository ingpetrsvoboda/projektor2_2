 ...
 * @author pes2704
 */
class Projektor2_View_HTML_Help_IP2 extends Projektor2_View_HTML_FormularPHP4 {
    ...
    echo '<H3>UKONČENÍ ÚČASTI V PROJEKTU A DOPLNĚNÍ IP2</H3>';

    echo '<form method="POST" action="index.php?akce=form&form=he_ukonceni_uc&save=1" name="form_ukonc">';
  ...
  <p>
   <?php 
    ...
    foreach ($aktivity as $druh=>$aktivita) {
        echo Projektor2_View_HTML_PlanFieldset::renderFieldsetKurz($pole, $druh, $this->context['kurzy_'.$druh], 'id', $aktivita['nadpis'], FALSE, $aktivita['s_certifikatem']);

//        echo Projektor2_View_HTML_PlanFieldset::renderFieldsetHodnoceni($pole, $druh, $aktivita);
    }
   ?> 
...
//TISK
   if ($pole['id_zajemce']){
        echo ('<p><input type="submit" value="Tiskni IP 2.část" name="pdf">&nbsp;&nbsp;&nbsp;</p> ');
